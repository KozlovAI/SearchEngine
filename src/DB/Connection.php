...
/**
 * Класс обертка над \Doctrine\DBAL\Connection
 */
class Connection
{
	/**
	...
	/**
	 * Возвращает инстант подключения к бд
	 *
	 * Если ранее уже было подключение к бд с переданными параметрами
	 * будет возвращено ранее созданное
	 * 
	 * @param string|array|\Doctrine\DBAL\Connection $dsn подключение к бд
	 * 
	 * @return Connection
	 */
	public static function getConnection($dsn)
	{
		if ($dsn instanceof DBAL\Connection) {
			$cache_key  = md5(serialize($connection->getParams())); 
		} else {
			if (!is_array($dsn)) {
				$dsn = self::parseDsn($dsn);
			}

			$cache_key  = md5(serialize($dsn));
		}

		if (!isset(self::$connections[$cache_key])) {
			self::$connections[$cache_key] = new self($dsn);
		}
	...
	public function __construct($dsn)
	{
		if ($dsn instanceof DBAL\Connection) {
			...
			if (is_array($dsn)) {
				...
				$connectionParams = self::parseDsn($dsn);
